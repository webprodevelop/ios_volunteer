...
public class ViewPager: NSObject {
    ...
    fileprivate func setupTabContainerView() {
        ...
        if #available(iOS 11.0, *) {
            ...
            tabContainer.bottomAnchor.constraint(equalTo: safeArea.bottomAnchor).isActive = true
        }
        else {
            let marginGuide = view.layoutMarginsGuide
            //tabContainer.topAnchor.constraint(equalTo: marginGuide.topAnchor).isActive = true
            tabContainer.bottomAnchor.constraint(equalTo: marginGuide.bottomAnchor).isActive = true
        }
    ...
    fileprivate func setupPageViewController() {
        let pageController = UIPageViewController(
            transitionStyle: options.viewPagerTransitionStyle,
        ...
        self.pageController?.view.bottomAnchor.constraint(equalTo: tabContainer.topAnchor).isActive = true
        self.pageController?.view.topAnchor.constraint(equalTo: view.topAnchor).isActive = true
    ...
    fileprivate func setupTabAndIndicator() {
        ...
        if options.isTabIndicatorAvailable {
            ...
            tabIndicator.topAnchor.constraint(equalTo: tabContainer.topAnchor).isActive = true
        ...
        if options.isTabHighlightAvailable {
            tabsViewList[currentPageIndex].addHighlight(options: self.options)
        }
    ...
    /// Sets up indicator for the page if enabled in ViewPagerOption.
    /// This method shows either tabIndicator or Highlights current tab or both.
    ...
    func showAnchor(index: Int) {
        if index >= tabsViewList.count { return }
        tabsViewList[index].showAnchor()
    }


    func hideAnchor(index: Int) {
        if index >= tabsViewList.count { return }
        tabsViewList[index].hideAnchor()
    }
