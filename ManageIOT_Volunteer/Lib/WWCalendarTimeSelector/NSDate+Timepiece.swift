...
extension Date {
    ...
    func change(weekday: Int) -> Date! {
        // - When first weekday is Sunday=1
        //return self + (weekday - i).days
        // - When first weekday is Monday=2 (not Sunday=1), then Sunday is assumed to be 8
        var i = self.weekday
        if i == 1 { i = 8 }
        return self + (weekday - i).days
    }
    ...
    public var beginningOfWeek: Date {
        ...
        // - When first weekday is Monday=2 (not Sunday=1)
        return change(weekday: 2).beginningOfDay
    }
